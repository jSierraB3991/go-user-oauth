...
func (s *GoOauthService) LoginWithTwoFactor(ctx context.Context, req gooauthrequest.GoLoginRequestTwoFactor) (*gooauthrest.JWT, error) {
	...
	if err != nil {
		s.saveInvalidDataLogin(ctx, req.Ip, req.UserAgent, userName, "Usuario no encontrado", true)
		return nil, err
	}

	if !user.IsActiveTwoFactorOauth {
		s.saveInvalidDataLogin(ctx, req.Ip, req.UserAgent, userName, "El usuario no tiene activado la doble autenticación", true)
	...
	if len(parts) != 2 {
		s.saveInvalidDataLogin(ctx, req.Ip, req.UserAgent, userName, "SecretData alterado, no tiene solo dos", true)
	...
	if !isValidCode {
		s.saveInvalidDataLogin(ctx, req.Ip, req.UserAgent, userName, "El código enviado dejop de estar habiliatdo + de: "+codeDecrypeted+" to + "+req.CodeTwoFactor, true)
